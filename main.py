...
def train():
    ...
    parser.add_argument("--check_point", type=str, default="/model_epoch_240.pwf",
                        help="Path of the pre-trained CNN")
    parser.add_argument("--device", type=str, default="cuda" if torch.cuda.is_available()
    else "cpu", help="Device (cuda or cpu)")
    parser.add_argument("--topo_size", type=int, default=50, help="Crop size for topo input")
    ...
    if args.train_type == 'unet-clstm':
        ...
        dataset = get_dataset_lstm(args) # (data, label)
        validation_split = .2
