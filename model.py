...
class RCN(nn.Module):
    def __init__(self, in_channels, out_channels, dataset):
        ...
        if self.dataset == 'CREMI':
            self.up_3 = nn.ConvTranspose2d(in_channels=128, out_channels=64, kernel_size=3,
                                           stride=2)  # for 1250 * 1250 kernel_size=3, stride=2
        else:
            self.up_3 = nn.ConvTranspose2d(in_channels=128, out_channels=64, kernel_size=2,
                                           stride=2)  # for 1250 * 1250 kernel_size=3, stride=2
...
if __name__ == "__main__":
    # A full forward pass
    im = torch.randn(2, 1, 50, 50)
    model = RCN(1, 32, 'CREMI')
